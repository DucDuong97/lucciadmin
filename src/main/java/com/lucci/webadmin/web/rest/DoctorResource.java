...
/**
 * REST controller for managing {@link com.lucci.webadmin.domain.Doctor}.
 */
@RestController
@RequestMapping("/api")
public class DoctorResource {
    ...
    public List<Doctor> getAllDoctors(@RequestParam(required = false) String filter) {
        if ("bookings-is-null".equals(filter)) {
            log.debug("REST request to get all Doctors where bookings is null");
            return doctorService.findAllWhereBookingsIsNull();
        }
