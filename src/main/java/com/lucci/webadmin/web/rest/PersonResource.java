...
/**
 * REST controller for managing {@link com.lucci.webadmin.domain.Person}.
 */
@RestController
@RequestMapping("/api")
public class PersonResource {
    ...
    public List<Person> getAllPeople(@RequestParam(required = false) String filter) {
        if ("doctor-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where doctor is null");
            return personService.findAllWhereDoctorIsNull();
        }
        if ("nurse-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where nurse is null");
            return personService.findAllWhereNurseIsNull();
        }
        if ("patient-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where patient is null");
            return personService.findAllWherePatientIsNull();
        }
        if ("receptionist-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where receptionist is null");
            return personService.findAllWhereReceptionistIsNull();
        }
        if ("accountant-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where accountant is null");
            return personService.findAllWhereAccountantIsNull();
        }
