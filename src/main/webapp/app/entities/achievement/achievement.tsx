import React, { useState, useEffect } from 'react';
import { connect } from 'react-redux';
import { Link, RouteComponentProps } from 'react-router-dom';
import { Button, Col, Row, Table } from 'reactstrap';
import { Translate, ICrudGetAllAction } from 'react-jhipster';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';

import { IRootState } from 'app/shared/reducers';
import { getEntities } from './achievement.reducer';
import { IAchievement } from 'app/shared/model/achievement.model';
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface IAchievementProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export const Achievement = (props: IAchievementProps) => {
  useEffect(() => {
    props.getEntities();
  }, []);

  const { achievementList, match, loading } = props;
  return (
    <div>
      <h2 id="achievement-heading">
        <Translate contentKey="lucciadminApp.achievement.home.title">Achievements</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="lucciadminApp.achievement.home.createLabel">Create new Achievement</Translate>
        </Link>
      </h2>
      <div className="table-responsive">
        {achievementList && achievementList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="global.field.id">ID</Translate>
                </th>
                <th>
                  <Translate contentKey="lucciadminApp.achievement.name">Name</Translate>
                </th>
                <th>
                  <Translate contentKey="lucciadminApp.achievement.number">Number</Translate>
                </th>
                <th>
                  <Translate contentKey="lucciadminApp.achievement.imgUrl">Img Url</Translate>
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {achievementList.map((achievement, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${achievement.id}`} color="link" size="sm">
                      {achievement.id}
                    </Button>
                  </td>
                  <td>{achievement.name}</td>
                  <td>{achievement.number}</td>
                  <td>{achievement.imgUrl}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${achievement.id}`} color="info" size="sm">
                        <FontAwesomeIcon icon="eye" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.view">View</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${achievement.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.edit">Edit</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${achievement.id}/delete`} color="danger" size="sm">
                        <FontAwesomeIcon icon="trash" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.delete">Delete</Translate>
                        </span>
                      </Button>
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </Table>
        ) : (
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="lucciadminApp.achievement.home.notFound">No Achievements found</Translate>
            </div>
          )
        )}
      </div>
    </div>
  );
};

const mapStateToProps = ({ achievement }: IRootState) => ({
  achievementList: achievement.entities,
  loading: achievement.loading,
});

const mapDispatchToProps = {
  getEntities,
};

type StateProps = ReturnType<typeof mapStateToProps>;
type DispatchProps = typeof mapDispatchToProps;

export default connect(mapStateToProps, mapDispatchToProps)(Achievement);
